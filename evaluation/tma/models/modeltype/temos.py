...
class TEMOS(BaseModel):
    ...
    def save_embeddings(self, batch):
        ...
        with torch.no_grad():
            ...
            # Emb_text=text_embedding
            # Emb_motion=motion_embedding
    ...
    def allsplit_step_single(self, split: str, batch, batch_idx):
        ...
        # Compute the losses
        loss = self.losses[split].compute_loss(
    ...
    def allsplit_epoch_end(self, split: str, outputs):
        ...
        if not self.trainer.sanity_checking:
            ...
            if self.trainer.is_global_zero:
                loss_str = " | ".join(f"{k}: {v:.4f}" for k, v in dico.items() if isinstance(v, float))
                import sys
                print(f"\n[Epoch {self.trainer.current_epoch}] [{split}] {loss_str}", file=sys.stderr, flush=True)

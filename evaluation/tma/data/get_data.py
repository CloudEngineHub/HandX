...
def get_mean_std(phase, cfg, dataset_name):
    ...
    if dataset_name=='interact':
        data_root = cfg.DATASET.INTERACT.ROOT
        mean = np.load(pjoin(data_root, 'mean_std_joint_pos_w_scalar_rot/mean.npy'))
        std = np.load(pjoin(data_root, 'mean_std_joint_pos_w_scalar_rot/std.npy'))
        # mean/std shape is (42, 4), motion is (T, 2, 21, 3), only take first 3 cols
        mean = mean[:, :3]
        std = std[:, :3]
        return mean,std
...
def get_datasets(cfg, logger=None, phase="train"):
    ...
    for dataset_name in dataset_names:
        if dataset_name.lower() in ["unimocap", "humanml3d", "kit",'interact']:
            ...
            # get WordVectorizer (interact dataset uses T5, not GloVe)
            if dataset_name == 'interact':
                wordVectorizer = None
            else:

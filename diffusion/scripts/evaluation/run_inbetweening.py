#!/usr/bin/env python3
"""
Motion Inbetweening Evaluation Script

This script evaluates temporal inpainting where the first and last frames
are fixed from ground truth, and the model generates the middle frames.

Masking Strategy:
- Temporal mask: First 10 frames (0-9) and last 10 frames (50-59) are fixed
- All joints are masked in the fixed regions
- Middle 40 frames (10-49) are generated
- Soft masking with transition width of 5 frames

This task tests the model's ability to generate smooth interpolation
between given start and end poses.
...
CHECKPOINTS = [
    {
        ...
        'eval_folder_name': 'generate_inbetweening',  # Customize output folder name
        'num_generated': 4,

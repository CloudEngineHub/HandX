#!/usr/bin/env python3
"""
Wrist Trajectory Conditioned Generation Evaluation Script

This script evaluates generation conditioned on wrist joint trajectories.
The wrist positions for both hands are fixed from ground truth, and the model
generates the remaining finger motions.

Masking Strategy:
- Spatial mask: Only wrist joints (indices 0 and 21) are fixed
- All 60 frames are masked for the wrist joints
- All other joints (fingers) are freely generated

This task tests the model's ability to generate realistic finger motions
that follow given wrist trajectories.

Usage:
    python run_wrist_traj.py
"""
...
CHECKPOINTS = [
    {
        ...
        'eval_folder_name': 'generate_wrist_traj',  # Customize output folder name
        'num_generated': 4,

...
class DistilbertEncoderBase(pl.LightningModule):
    ...
    def __init__(self, modelpath: str, finetune: bool = False):
        ...
        TOP_L = 6
        num_layers = len(self.text_model.base_model.encoder.block)
        keep_layers = set(range(num_layers - TOP_L, num_layers)) 
        for name, param in self.text_model.named_parameters():
            if 'lora_' in name:
                m = re.search(r"encoder\.block\.(\d+)\.", name)
                layer_ok = (m is not None) and (int(m.group(1)) in keep_layers)
                param.requires_grad = bool(layer_ok)
        ...
        # self.text_model = SentenceTransformer("sentence-transformers/sentence-t5-base")

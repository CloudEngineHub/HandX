...
def evaluation_vqvae_motionmillion_1gpu(out_dir, train_loader, val_loader, net, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, best_mpjpe, comp_device, draw = True, save = True, savegif=False, savenpy=False, fps=60, cal_acceleration=False) : 
    ...
    for i,batch in tqdm(enumerate(val_loader)):
        ...
        for i in range(bs):
            ...
            gpe += torch.mean(torch.linalg.norm(pose_xyz[:, :m_length[i]] - pred_xyz[:, :m_length[i]], dim=-1)).cpu()
    ...
    print("mpjpe: ", mpjpe,'gpe: ',gpe)

    motion_indices = [motion_index.flatten() for motion_index in motion_indices]
    
    msg = f"--> \t Eva. Iter {nb_iter} :, MPJPE. {mpjpe}, 'GPE:' {gpe} "
    if cal_acceleration:
    ...
    if cal_acceleration:
        return best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, best_mpjpe, writer, logger, pred_mean_acceleration_seq, pred_max_acceleration_seq, gt_mean_acceleration_seq, gt_max_acceleration_seq,gts_pred,recovers_pred
    else:
        return best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, best_mpjpe, writer, logger,gts_pred,recovers_pred
...
def calculate_mpjpe(gt_joints, pred_joints):
    ...
    # Compute MPJPE
    mpjpe = torch.linalg.norm(pred_joints_l - gt_joints_l, dim=-1) # num_poses x num_joints=22
    mpjpe_seq_l = mpjpe.mean(-1) # num_poses
...
def calculate_mpjpe_global(gt_joints, pred_joints):
    """
    gt_joints: num_poses x num_joints(22) x 3
    pred_joints: num_poses x num_joints(22) x 3
    (obtained from recover_from_ric())

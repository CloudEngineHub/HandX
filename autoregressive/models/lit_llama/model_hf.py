...
llama_configs = {
    ...
    "222M": dict(n_layer=16, n_head=16, n_embd=1024),
...
class Block(nn.Module):
    def __init__(self, config: LLaMAHFConfig) -> None: # , use_qkNorm=False, use_moe=False) -> None:
        ...
        # if use_moe:
        #     # n_embed, num_experts, top_k, dropout
        #     # self.smoe = SparseMoE(config.n_embd, config.n_experts, config.top_k, 0.1)
        #     self.smoe = Qwen2MoeSparseMoeBlock(config)
        # else:
        #     self.mlp = MLP(config)
        self.mlp = MLP(config)

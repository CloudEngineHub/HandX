...
def merge_into_pickle(root_dir, vq_name):

    with open(pjoin(_DATA_DIR, 'texts_all.pkl'),'rb') as f:
        text_data = pickle.load(f)
    all_files = list(text_data.keys())
    # create data dict
    data_dict = {
        "text_data": {},
        "code_data": {},
    ...
    for name in tqdm(all_files):
        code_file_path = os.path.join(root_dir, vq_name, name + ".npy")
        ...
        data_dict["text_data"][name] = text_data[name]
        data_dict["code_data"][name] = code_data

    print(f"Skipped {skipped} files, kept {len(data_dict['file_names'])} files")

    with open(os.path.join(root_dir, f"{vq_name}.pkl"), "wb") as f:
...
if __name__ == '__main__':
    ...
    merge_into_pickle(root_dir, args.vq_name)            
